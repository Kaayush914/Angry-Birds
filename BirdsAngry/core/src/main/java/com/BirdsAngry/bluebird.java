...
//
//    public yellowbird(String name, int posx,int posy, int velocity, int width, int height) {
//        super(name,posx,posy,velocity,width,height);
//        this.name = name;
//        this.posx = posx;
//        this.posy = posy;
...
public class bluebird extends Birds implements Serializable {
    private String name;
    private int posx;
    private int posy;
    private int width;
    private int height;
    private int velocity;
    private transient Texture bluebird;
    private transient TextureRegion bluebirdRegion;
    ...
    public bluebird(String name, int posx,int posy, int velocity, int width, int height) {
        ...
        this.velocity = velocity;
        bluebird= new Texture("bluebird.png");
        bluebirdRegion= new TextureRegion(bluebird);
...
//                }
            if (newY <= 150) { // Assuming 70 is the ground level
                newY = 150; // Ensure it stops at ground level
...
//            trajectoryHandler.calculateTrajectory(dragStart, new Vector2(dragStart.x - dragEnd.x, dragStart.y - dragEnd.y));
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        batch.draw(bluebirdRegion, getX(), getY(),getWidth()/2,getHeight()/2,getWidth(), getHeight(),1,1,getRotation());
...
//    public void split(Stage stage) {
//        {
//            // Split into 3 birds
//            float offsetX = 20;
//            float offsetY = 20;
//
//            bluebird bird1 = new bluebird("Blue", (int) (this.getX() - offsetX), (int) (this.getY() + offsetY), this.getBirdVelocity(), (int) this.getWidth(), (int) this.getHeight());
//            bluebird bird2 = new bluebird("Blue", (int) (this.getX() + offsetX), (int) (this.getY() + offsetY), this.getBirdVelocity(), (int) this.getWidth(), (int) this.getHeight());
//
//            stage.addActor(bird1);
//            // take up the birds velocoity and continue the motion
//            bird1.currentVelocity = this.currentVelocity;
//            bird1.isLaunched = true;
////            bird1.elapsedTime = this.elapsedTime;
//            stage.addActor(bird2);
//            bird2.currentVelocity = this.currentVelocity;
//            bird2.isLaunched = true;
////            bird2.elapsedTime = this.elapsedTime;
//            stage.addActor(bird1);
//
//        }
//    }

    public void removesplitbirds(Birds bird1, Birds bird2){
        bird1.remove();
        bird2.remove();
